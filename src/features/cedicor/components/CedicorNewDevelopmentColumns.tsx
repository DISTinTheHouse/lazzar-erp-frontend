...
import { CedicorNewDevelopmentTimelineDialog } from "./CedicorNewDevelopmentTimelineDialog";
import type {
  NewDevelopment,
  FlowStatus,
...
const ESTATUS_CFG: Record<FlowStatus, { cls: string; dot: string }> = {
...
const CEDICOR_NEW_DEVELOPMENT_STEPPER_CFG = {
  ...
  TableMiniStepperProps<FlowStatus>,
...
const EstatusBadge = ({ estatus }: { estatus: FlowStatus }) => {
...
const VerificacionBadge = ({
  ...
  estado: MaterialVerificationStatus;
}) => {
...
const ActionsCell = ({ row }: { row: NewDevelopment }) => {
  ...
  return (
    <>
      ...
      <CedicorNewDevelopmentTimelineDialog
        order={row}
        open={isTimelineOpen}
...
const col = createColumnHelper<NewDevelopment>();

export function getCedicorNewDevelopmentColumns(): ColumnDef<NewDevelopment>[] {
  return [
    col.accessor('folio', {
      header: 'Folio',
      meta: { label: 'Folio' },
      ...
      cell: (info) => {
        ...
        return (
          <TableMiniStepper
            ...
            status={row.estatus}
            {...CEDICOR_NEW_DEVELOPMENT_STEPPER_CFG}
          />
        );
      },
      size: 160,
    ...
    col.accessor('estatus', {
      header: 'Estatus',
      meta: { label: 'Estatus' },
      cell: (info) => <EstatusBadge estatus={info.getValue<FlowStatus>()} />,
      size: 200,
    }),
